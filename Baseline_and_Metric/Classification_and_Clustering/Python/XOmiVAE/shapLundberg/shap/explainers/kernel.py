...
log = logging.getLogger('shapLundberg')
...
class KernelExplainer(Explainer):
    ...
    data : numpy.array or pandas.DataFrame or shapLundberg.common.DenseData or any scipy.sparse matrix
    ...
    def __init__(self, model, data, link=IdentityLink(), **kwargs):
        ...
        if len(self.data.weights) > 100:
            log.warning("Using " + str(len(self.data.weights)) + " background data samples could cause " +
                        "slower run times. Consider using shapLundberg.sample(data, K) or shapLundberg.kmeans(data, K) to " +
