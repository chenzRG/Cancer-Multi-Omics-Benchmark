...
def bar(shap_values, max_display=10, order=order.abs, clustering=None, cluster_threshold=0.5, show=True):
    ...
    Parameters
    ----------
    shap_values : shapLundberg.Explanation
    ...
    assert str(type(shap_values)).endswith(
        "Explanation'>"), "The shap_values paramemter must be a shapLundberg.Explanation object!"
    ...
    if partition_tree is not None:
        assert partition_tree.shape[
                   1] == 4, "The clustering provided by the Explanation object does not seem to be a partition tree (which is all shapLundberg.plots.bar supports)!"
    ...
    if num_features < len(shap_values):
        ...
        shap_values[feature_order[num_features - 1]] = np.sum(
            [shap_values[feature_order[i]] for i in range(num_features - 1, len(shap_values))])
    ...
    if partition_tree is not None:
        ...
        l = pl.axvline((cluster_threshold / (xlines_max - xlines_min)) * 0.1 * (xmax - xmin) + xmax, color="#dddddd",
                       dashes=(1, 1))
        l.set_clip_on(False)
