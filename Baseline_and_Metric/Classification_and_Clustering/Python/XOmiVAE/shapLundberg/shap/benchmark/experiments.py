...
def run_remote_experiments(experiments, thread_hosts, rate_limit=10):
    ...
    experiments : iterable
        Output of shapLundberg.benchmark.experiments(...).
    ...
    for host in set(thread_hosts):
        ...
        try:
            subprocess.run(["ssh", hostname, "pkill -f shapLundberg.benchmark.run_experiment"], timeout=15)
...
def __run_remote_experiment(experiment, remote, cache_dir="/tmp", python_binary="python"):
    ...
    #start = time.time()
    cmd = "CUDA_VISIBLE_DEVICES=\"\" "+python_binary+" -c \"import shapLundberg; shapLundberg.benchmark.run_experiment(['%s', '%s', '%s', '%s'], cache_dir='%s')\" &> %s/%s.output" % (
