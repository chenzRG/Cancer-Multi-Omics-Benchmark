...
def force_plot(base_value, shap_values, features=None, feature_names=None, out_names=None, link="identity",
    ...
    if (type(base_value) == np.ndarray or type(base_value) == list):
        if type(shap_values) != list or len(shap_values) != len(base_value):
            raise Exception("In v0.20 force_plot now requires the base value as the first parameter! " \
                            "Try shapLundberg.force_plot(explainer.expected_value, shap_values) or " \
                            "for multi-output models try " \
                            "shapLundberg.force_plot(explainer.expected_value[0], shap_values[0]).")
    ...
    if shap_values.shape[0] == 1:
        ...
        if shap_values.shape[0] > 3000:
            warnings.warn("shapLundberg.force_plot is slow for many thousands of rows, try subsampling your data.")
...
def initjs():
    assert have_ipython, "IPython must be installed to use initjs()! Run `pip install ipython` and then restart shapLundberg."
...
def save_html(out_file, plot, full_html=True):
    ...
    plot : BaseVisualizer
        Visualizer returned by shapLundberg.force_plot()
    ...
    assert isinstance(plot, BaseVisualizer), "save_html requires a Visualizer returned by shapLundberg.force_plot()."
    internal_open = False
...
class SimpleListVisualizer(BaseVisualizer):
    ...
    def html(self):
        # assert have_ipython, "IPython must be installed to use this visualizer! Run `pip install ipython` and then restart shapLundberg."
        return """
