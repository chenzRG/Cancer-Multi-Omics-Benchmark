...
def baseline_mdi(data, args, log_path):
    ...
    result_str = 'rmse: {:.3g}, mae: {:.3g}\n'.format(rmse, mae)
    log_path = './logs/'
    if not os.path.isdir(log_path):
        os.makedirs(log_path)
    with open(log_path + 'results.txt', 'a') as f:
        f.write(result_str)
    obj = {'rmse': rmse, 'mae': mae}
    with open(log_path + 'result.pkl', 'wb') as f:
        pickle.dump(obj, f)
    print('Results saved to {}results.txt and {}result.pkl'.format(log_path, log_path))
