...
def perform_kegg_pathway_analysis(results, organism='hsa', gene_column='log2FC', p_value_cutoff=0.05):

#     Performs KEGG pathway analysis using the differential expression results.
    
#     Parameters:
#     results (DataFrame): DataFrame with log2 fold change and adjusted p-values for each gene.
#     organism (str): KEGG organism code (e.g., 'hsa' for human).
#     gene_column (str): Column name for the gene scores.
#     p_value_cutoff (float): p-value cutoff for selecting significant genes.
    
#     Returns:
#     DataFrame: DataFrame with KEGG pathway enrichment results.
    ...
    enrichment_results = gp.enrichr(gene_list=gene_scores.keys(),
